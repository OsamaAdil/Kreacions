...
router.post('/', formatRequest, function (req, res, next) {
  ...
  data.req = req.data;
  data.body = req.body;
  createActivityLogs(data, function(err, response) {
      if (err) {
...
router.get('/', formatRequest, function (req, res, next) {
  ...
  getActivityLog(data, function(err, response) {
      if (err) {
          console.log("entered into error", err);
          return res.status(err.status).send(err);
      }
...
router.patch('/', formatRequest, function (req, res, next) {
  ...
  editActivityLogs(data, function(err, response) {
      if (err) {
          console.log("entered into error", err);
          return res.status(err.status).send(err);
      }
...
router.delete('/', formatRequest, function (req, res, next) {
  ...
  deleteActivityLogs(data, function(err, response) {
      if (err) {
          console.log("entered into error", err);
          return res.status(err.status).send(err);
      }
